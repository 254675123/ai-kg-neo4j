...
import teacher_question_processor
import sys
reload(sys)
...
class AutoCorrelation:
    ...
    word net generator.
    """
    def __init__(self, course_scop_file, course_source_filename='course-20181026.txt', istest=False):
        """
        ...
        # 老师标注的知识点
        self.teacher_processor = teacher_question_processor.TeacherQuestionProcessor()
        rootpath = '{}/../../data/course-knowledge-teacher'.format(self.__curpath)
        self.teacher_processor.isTest = istest
        self.teacher_processor.get_course_exam_dict(rootpath)
        # course 处理器
    ...
    def questionAndKnowledge(self, course_path_info_list):
        ...
        aq.teacher_processor = self.teacher_processor
        ...
        # 保留统计结果
        self.exam_processor.createCypherFile(course_path_info_list,aq.exam_question_group_dict)
    ...
    def saveProcessedCourse(self,rootpath, course_base_code):
        output_mid_filepath = '{}/course-processed.txt'.format(rootpath)
    ...
    def associateFlow(self, course_list):
        ...
        self.treefactory.teacher_processor = self.teacher_processor
    ...
    def setCourseBaseDict(self, c_rootpath):
        ...
        print '处理总数：{}'.format(count)
        print '加入基础课程的总数：{}'.format(count_add)
...
if __name__ == "__main__":
    # 该文件放在course-base下面
    course_scop_txt_file = 'course-scope-800plus-combine.xlsx'
    # course_source_filename = 'course-base-20181207.txt'
    course_source_filename = 'course-base-20190211.txt'
    # 该文件夹放在course-knowledge-machine下面
    dirname = '20181217-800plus-combine'
    #dirname = '20181210-bug-test'
    test = False
    sr = AutoCorrelation(course_scop_txt_file, course_source_filename, istest=test)
