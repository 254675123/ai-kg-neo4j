...
class ExamQuestionProcessor:
    ...
    def createCypherFile(self, course_path_info_list):
        """
        数据生成cypher语句文件
        :param course_path_info_list: 
        :return: 
        """
        cypher_statement_list = []
        for course_path_info in course_path_info_list:
            if not self.exam_info.examquestion_dict.__contains__(course_path_info.course):
                continue

            exam_question_list = self.exam_info.examquestion_dict.get(course_path_info.course)
            # 将知识点和试题关联起来
            cypherlist = self.generateCypher(course_path_info.course,exam_question_list)
            cypher_statement_list = cypher_statement_list + cypherlist
        # 保存数据
